...
def es_letra_castellana(letra):
    return (letra >= 'A' and letra <= 'Z') or (letra >= 'a' and letra <= 'z') or letra in 'ÑñÁáÉéÍíÓóÚúÜü'

def primer_no_castellano(cadena):
    i = 0
    es_correcta = True
    while i < len(cadena) and es_correcta:
        if not es_letra_castellana(cadena[i]):
            return cadena[i]
            es_correcta = False
        i += 1
...
while palabra != '':
    ...
    while i < len(palabra) and es_correcta:
        if not es_letra_castellana(palabra[i]):
            print('Contiene un carácter que no es del alfabeto castellano: \'{0}\''.format(primer_no_castellano(palabra)))
            es_correcta = False
        i += 1
    if es_correcta:
